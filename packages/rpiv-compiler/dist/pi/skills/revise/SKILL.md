...
2. **Handle missing input**:
   - If no plan, offer recent plans via ask_user_question.
...
1. **Spawn parallel agents** for research:
   - Agent({ subagent_type: "codebase-locator", description: "analyze codebase-locator", prompt: "$PROMPT" }) / Agent({ subagent_type: "codebase-analyzer", description: "analyze codebase-analyzer", prompt: "$PROMPT" }) / Agent({ subagent_type: "codebase-pattern-finder", description: "analyze codebase-pattern-finder", prompt: "$PROMPT" }) for code investigation.
   - Agent({ subagent_type: "artifacts-locator", description: "analyze artifacts-locator", prompt: "$PROMPT" }) / Agent({ subagent_type: "artifacts-analyzer", description: "analyze artifacts-analyzer", prompt: "$PROMPT" }) for historical context.
...
2. **Confirm plan edits** via ask_user_question. Options: "Proceed (Recommended)"; "Adjust approach"; "Show me first".
