...
{{dispatch:artifact-code-reviewer}}
- Prompt: Review the finalized plan against the live codebase at HEAD. Walk every Phase code fence, audit against code-quality / codebase-fit / actionability, emit one severity-tagged row per finding.

{{dispatch:artifact-coverage-reviewer}}
- Prompt: Review the finalized plan's verification-intent coverage. Walk every ## Verification Notes and ## Precedents & Lessons entry; for each, verify it lands in either a phase's ### Success Criteria: bullet or as a visible code mirror. Emit one severity-tagged row per uncovered entry.
...
1. **Triage Step 4 reviewer findings first** (skip if Step 4 returned no findings):
   ...
   Use {{tool:ask_user}} with options "applied / deferred / dismissed":
   ...
   **Order and batching**: blockers sequentially (resolution may invalidate later rows). Concerns and suggestions: batch up to 4 independent rows per {{tool:ask_user}} call. Independent = different files / different intents AND neither recommendation references the other's location; otherwise sequential.
...
|---|---|
| Step 4 post-finalization code review (mandatory) | {{dispatch:artifact-code-reviewer}} |
| Step 4 post-finalization coverage review (mandatory) | {{dispatch:artifact-coverage-reviewer}} |
