...
2. **Identify Ambiguities (Wave 1 Dispatch)**:
   - Spawn parallel agents to fill gaps in the research:
     - {{dispatch:codebase-pattern-finder}} to find canonical examples for the proposed change.
     - {{dispatch:integration-scanner}} to confirm outbound dependencies.
     - {{dispatch:precedent-locator}} to check for similar architectural decisions.
   - Wait for all agents to complete.

3. **Developer Checkpoint**:
   - Present the identified approach and any remaining ambiguities via {{tool:ask_user}}.
...
5. **Generate Slice Code**:
   ...
   - Use {{dispatch:slice-verifier}} for each slice to catch cross-slice mismatches.
   - Present condensed micro-checkpoints for each slice via {{tool:ask_user}}.
