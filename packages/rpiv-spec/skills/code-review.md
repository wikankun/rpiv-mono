...
1. **Resolve scope via the bundled helper.** Determine the scope spec from the value the user supplied (visible in `## Input` above as the substituted argument). If empty, use the literal string `auto`; if ambiguous (prose, mixed list, unrecognised branch name), clarify via {{tool:ask_user}} — options: (A) "review current branch vs default branch (first-parent)" → `auto`, (B) "review every tracked change vs HEAD (staged + unstaged)" → `modified`, (C) "review unstaged changes only" → `working`, (D) "restate scope" → free-text — then re-invoke. Then run:

   ```bash
   node "${SKILL_DIR}/_helpers/review-range.mjs" "<scope-spec>"
   ```

   The helper emits labeled key/value lines (`default_branch:`, `strategy:`, `oldest:`, `newest:`, `base:`, `tip:`, `range:`, `fp_flag:`, `patch_path:`) followed by a `---changed-files---` block. Read those as authoritative for the rest of Step 1. If `strategy: unrecognised` appears, the `note:` field explains why — clarify via {{tool:ask_user}} and re-invoke with a valid spec.
   ...
   | anything else (prose, mixed list, unresolvable ref) | clarify via {{tool:ask_user}}, then re-invoke |
...
2. **Developer checkpoint**:
   - Present the reconciled findings to the developer via {{tool:ask_user}}
