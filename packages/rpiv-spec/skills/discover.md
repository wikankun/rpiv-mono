...
1. **Ask one open-ended `intent` question** via {{tool:ask_user}}:
...
2. **Spawn the chosen agent(s) in parallel using the Agent tool.** Draft each prompt yourself from the developer's stated intent — keep the slice narrow (one component, one seam) and avoid breadth phrasing like "everything related to X".
...
3. **Ask via {{tool:ask_user}}.** Lead with the recommended option. The "Other" option is automatic and handles open-ended answers.
...
- **Always one question at a time**: Even with 2-4 batched independent `detail` leaves, that's still one {{tool:ask_user}} call — wait for answers before asking the next round.
