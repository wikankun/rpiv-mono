...
Ask the developer about aesthetic direction across 7 dimensions. Use {{tool:ask_user}} — one question at a time, wait for the answer before asking the next. Lead each question with the recommended option labeled `(Recommended)`.
...
Emit the complete guidelines as your own assistant message. Do NOT write to a file. The guidelines become part of the conversation transcript — they survive compaction and inform all subsequent turns.
...
- **One question at a time**: Use {{tool:ask_user}} for each dimension individually. Never batch multiple dimensions into one call.
