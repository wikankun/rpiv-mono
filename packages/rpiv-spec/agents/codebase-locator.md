...
3. **Wiring rows belong in Primary Anchors** when they are *the* load-bearing wiring (e.g., the `session_start` binding for a session-start feature). Don't dilute the section with every `[doc]` or `[use]`.
