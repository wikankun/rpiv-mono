...
2. **If no parameters provided**, branch on the `recent handoffs:` listing in the Metadata block:
   ...
   - **Exactly one entry** — confirm with ask_user: "Resume this handoff?" with options "Resume `<filename>` (Recommended)" and "Pick a different path".
   - **Two or more entries** — present the top 4 filenames as ask_user options.
...
2. **Spawn focused research agents**:
   ...
   - @artifacts-analyzer to summarize mentioned artifacts.
   - @codebase-analyzer to verify recent changes and learnings still apply.
...
2. **Confirm approach** via ask_user. Options: "Proceed (Recommended)"; "Adjust approach"; "Re-analyze".
