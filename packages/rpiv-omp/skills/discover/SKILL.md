...
1. **Ask one open-ended `intent` question** via ask_user:
...
3. **Ask via ask_user.** Lead with the recommended option. The "Other" option is automatic and handles open-ended answers.
...
- **Always one question at a time**: Even with 2-4 batched independent `detail` leaves, that's still one ask_user call — wait for answers before asking the next round.
