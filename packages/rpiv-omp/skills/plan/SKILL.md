...
1. **Triage Step 4 reviewer findings first** (skip if Step 4 returned no findings):
   ...
   Use ask_user with options "applied / deferred / dismissed":
   ...
   **Order and batching**: blockers sequentially (resolution may invalidate later rows). Concerns and suggestions: batch up to 4 independent rows per ask_user call. Independent = different files / different intents AND neither recommendation references the other's location; otherwise sequential.
