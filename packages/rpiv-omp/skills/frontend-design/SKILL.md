...
Ask the developer about aesthetic direction across 7 dimensions. Use ask_user — one question at a time, wait for the answer before asking the next. Lead each question with the recommended option labeled `(Recommended)`.
...
- **One question at a time**: Use ask_user for each dimension individually. Never batch multiple dimensions into one call.
