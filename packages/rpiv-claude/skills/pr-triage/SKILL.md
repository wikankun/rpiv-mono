...
1. **Resolve and Fetch**:
   ...
   - If ambiguous, clarify via ask_user.
...
3. **Dispatch Assessment (Wave 1 parallel agents)**:
   - @diff-auditor for security scan on the raw patch.
   - @codebase-analyzer for convention drift vs the discovered standards.
   - @codebase-analyzer for intent vs diff + scope-creep check.
4. **Triage Checkpoint**:
   - Tally findings (security tier, blockers, drift).
   - Present Top Blockers and recommended disposition (Review · Request changes · Hold · Decline) via ask_user.
